...
export function NostrEventCardHeader({ event, compact = false, className }: EventCardHeaderProps) {
    ...
    <div className={cn("flex items-center justify-between", className)}>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className={cn("text-zinc-500 dark:text-zinc-400 !mb-0 !pb-0", compact ? "text-[10px]" : "text-xs")}>
            @{username} · {timestamp}
          </p>
        </div>
      </div>
