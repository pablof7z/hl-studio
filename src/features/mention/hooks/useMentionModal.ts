...
export function useMentionModal({ onSelect, onClose }: UseMentionModalProps = {}) {
    ...
    const { users, eose: userEose } = useNostrUserSearch(searchType === 'user' ? query : '');

    const eventSearchResult = useNostrEventSearch(searchType === 'event' ? query : '');
    ...
    useEffect(() => {
        ...
        const isEventIdentifier =
            query.startsWith('nevent1') || query.startsWith('note1') || query.startsWith('naddr1');
