        ...
        const sub = ndk.subscribe([
            ...
            onEvent: (event) => {
                ...
                draft.getEvent().then(async (innerEvent: NDKEvent | null | undefined) => {
                    if (!innerEvent) {
                        console.warn('Draft event does not have an inner event:', event.inspect);
                        return;
                    }
                    ...
                    if (!draftId) {
                        console.warn('Draft event does not have a draft ID:', event.inspect);
                        return;
                    }
